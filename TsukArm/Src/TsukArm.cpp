/****************************************************
/* TsukArm class for Arduino platform
/* Author: MTM Systems CO.,Ltd.
/* Date: 31 Aug. 2016
/*
/* Version 1.01
/* www.mtmsystems.jp
/*  
/* Description: this lib refers to 7bot-lib.
/* 
/*
/***************************************************/
#include "TsukArm.h"
...
void TsukArm::setStoreData() {
  dueFlashStorage.write(0, 8);  // TsukArm
...
// Set TsukArm to forceless mode, which you can drag TsukArm by hand easily and also read stable pose feedbacks
void TsukArm::forcelessMode() {
...
// Set TsukArm to stop mode, which you can drag TsukArm by hand but the robot still have some forces.
// This mode is useful for device protection, robot can be moved by external forces, but still have some resistance.
void TsukArm::stopMode() {
  if (Servos[0].attached()) {
    for (int i = 0; i < SERVO_NUM; i++) {
...
// TsukArm move fuction
// Given each axis angle(Unit:degrees)
void TsukArm::move(double angles[SERVO_NUM]) {
...
// Geometry constrains
void TsukArm::geometryConstrain() {
  if (pos[1] < 0) {
    pos[1] = 0;
...
void TsukArm::calculateServoPosD() {
  for (int i = 0; i < SERVO_NUM; i++) {
...
PVector TsukArm::zAxiRotate(PVector point, double _angle) {
  PVector pt;
  pt = PVector( cos(_angle) * point.x - sin(_angle) * point.y, sin(_angle) * point.x + cos(_angle) * point.y, point.z );
  return pt;
}

PVector TsukArm::arbitraryRotate(PVector point, PVector pointA, PVector pointB, double _angle) {
...
PVector TsukArm::calcProjectionPt(PVector pt0, PVector pt1, PVector nVec) {
...
int TsukArm::IK5(PVector j6, PVector vec56_d) {
  int status = -1;
...
// return: [0]no error; [1]IK3 out of valid range; [2]IK5-theta4 out of range;
int TsukArm::IK6(PVector j6, PVector vec56_d, PVector vec67_d) {
  int status = -1;
...
void TsukArm::btDetectionAndBuz() {
